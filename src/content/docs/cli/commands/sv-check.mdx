...
Инструмент Vue [VTI](https://github.com/vuejs/vetur/tree/master/vti), который лег в основу ёsvelte-checkё

## Часто задаваемые вопросы и ответы

### Почему нет опции для проверки отдельных файлов (например, только изменённых)?

`svelte-check` требует полного обзора проекта для корректной работы. Например, если переименовать пропс компонента, но не обновить его использования — эти места станут ошибками, но они будут пропущены при проверке только изменённых файлов.
