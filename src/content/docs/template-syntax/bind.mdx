...
Общая синтаксическая структура — `bind:property={expression}`, где `expression` является _lvalue_ (т. е. переменной или свойством объекта). Когда выражение является идентификатором с тем же именем, что и свойство, мы можем опустить выражение — другими словами, эти два варианта эквивалентны:
