...
В случае числового ввода (`type="number"` или `type="range"`) значение будет преобразовано в число:
...
Флажки можно связывать с помощью `bind:checked`:
...
:::note
Используйте `bind:group` для переключателей вместо `bind:checked`.
:::

## `<input bind:indeterminate>`

Флажки могут находиться в [неопределённом](https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement/indeterminate) состоянии, независимо от того, отмечены они или нет:
...
<Tabs syncKey="lang">
  <TabItem label="JS">
    ...
    <p>Тортилья: {tortilla}</p>
    <p>Начинка: {fillings.join(', ') || 'Нет'}</p>
...
Начиная с версии 5.57.0, если `<select>` имеет `defaultValue` и является частью формы, при сбросе формы он вернётся к этому значению, а не к пустой строке. Обратите внимание, что при первоначальном рендеринге значение привязки имеет приоритет, если только оно не равно `null` или `undefined`.
