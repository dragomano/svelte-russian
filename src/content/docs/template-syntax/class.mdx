...
Начиная со Svelte 5.19, Svelte также предоставляет тип `ClassValue`, который представляет собой тип значения, принимаемого атрибутом `class` для элементов. Это полезно, если вы хотите использовать типобезопасное имя класса в пропсах компонента:
