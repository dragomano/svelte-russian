...
Если для привязываемого свойства задано резервное значение, родительский компонент должен передать значение, отличное от `undefined`, если он использует `bind:`. Это устраняет неоднозначность относительно того, какое значение следует использовать, поскольку родительский и дочерний компоненты должны совместно использовать одно и то же значение при привязке.
