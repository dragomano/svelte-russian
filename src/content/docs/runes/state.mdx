...
:::note
Экземпляры классов не проксируются. Вы можете создавать [реактивные поля состояния](#классы) в классах, которые определяете. Svelte предоставляет реактивные реализации встроенных типов, таких как `Set` и `Map`, которые можно импортировать из [`svelte/reactivity`](https://svelte.dev/docs/svelte/svelte-reactivity).
:::
...
Вы также можете использовать `$state` в полях класса (как публичных, так и приватных) или в качестве первого присваивания свойства непосредственно внутри `constructor`:
