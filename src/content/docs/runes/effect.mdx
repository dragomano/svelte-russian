...
`$effect.pre` выполняется до обновлений DOM, запланированных после него, а не до каждой мутации DOM в рамках одного цикла обновления — DOM родительских компонентов уже может быть обновлён. При использовании [выражений `await`](/template-syntax/await-expressions) обновления блоков вроде `{#if ...}` и `{#each ...}` в том же компоненте также выполняются до `$effect.pre`.
