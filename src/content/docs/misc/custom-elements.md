...
- `shadow`: необязательное свойство для настройки shadow root. Принимает следующие значения:
  - `"none"`: shadow root не создаётся (стили перестают быть изолированными, слоты недоступны).
  - `"open"`: создаётся shadow root с опцией `mode: "open"`.
  - [`ShadowRootInit`](https://developer.mozilla.org/en-US/docs/Web/API/Element/attachShadow#options): объект с настройками, который будет передан в метод `attachShadow()`.
