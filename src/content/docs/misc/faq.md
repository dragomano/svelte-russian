...
Официальная библиотека маршрутизации — это [SvelteKit](https://svelte.dev/docs/kit). SvelteKit предоставляет файловый маршрутизатор, серверный рендеринг (SSR) и горячую перезагрузку модулей (HMR) в одном удобном пакете. Он имеет сходство с Next.js для React. SvelteKit также поддерживает маршрутизацию на основе хэша для клиентских приложений.
...
Хотя большинство мобильных приложений пишутся без использования JavaScript, если вы хотите использовать свои существующие компоненты Svelte и знания о Svelte при создании мобильных приложений, вы можете превратить [SvelteKit SPA](/kit/build-and-deploy/single-page-apps) в мобильное приложение с помощью [Tauri](https://v2.tauri.app/start/frontend/sveltekit/) или [Capacitor](https://capacitorjs.com/solution/svelte). Мобильные функции, такие как камера, геолокация и push-уведомления, доступны через плагины для обеих платформ.

Вы также можете создавать приложения на Svelte, компилируемые в нативные компоненты, с помощью [Symbiote Native](https://docs.symbiote-native.dev/), использующего инфраструктуру React Native.

Работа над [поддержкой пользовательских рендереров в Svelte 5](https://github.com/sveltejs/svelte/issues/15470) завершена, но эта возможность ещё не была объединена. API пользовательского рендеринга позволит обеспечить поддержку дополнительных мобильных фреймворков, таких как Lynx JS и Svelte Native. Symbiote Native также перейдёт на этот API. Svelte Native был доступен в качестве варианта для Svelte 4, но Svelte 5 в настоящее время его не поддерживает. Svelte Native позволяет создавать приложения NativeScript с использованием компонентов Svelte, содержащих [компоненты пользовательского интерфейса NativeScript](https://docs.nativescript.org/ui/), а не DOM-элементы.
