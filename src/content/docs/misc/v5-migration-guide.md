...
- Пробелы в начале и в конце тега полностью удаляются
  ...
  Это новое поведение немного отличается от нативного рендеринга HTML. Например, `<p>foo<span> - bar</span></p>` будет отображаться так:

  - `foo - bar` в HTML
  - `foo- bar` в Svelte 5

  Вы можете вернуть пропущенный пробел, переместив его за пределы `<span>`...

  ```svelte
  <p>foo <span>- bar</span></p>
  ```

  ...или, если это необходимо по причинам стилизации, включите его как выражение:
...
### События касания являются пассивными

При использовании атрибутов событий `ontouchstart` и `ontouchmove` обработчики по умолчанию считаются [пассивными](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener#using_passive_listeners), чтобы соответствовать поведению браузера по умолчанию. Это значительно улучшает отзывчивость, позволяя браузеру немедленно начинать прокрутку документа, вместо того чтобы ждать, вызовет ли обработчик события метод `event.preventDefault()`.
