...
Ранее Svelte не анализировал селекторы внутри `:is(...)`, `:has(...)` и `:where(...)`, фактически рассматривая их как глобальные. Svelte 5 анализирует их в контексте текущего компонента. Таким образом, некоторые селекторы теперь могут рассматриваться как неиспользуемые, если они полагались на это поведение. Чтобы исправить это, используйте `:global(...)` внутри селекторов `:is(...)/:has(...)/:where(...)`.
