...
Чтобы безопасно работать с этим, можно напрямую передать тип в качестве параметра обобщённого типа. TypeScript выведет для переменной тип `number | undefined`.
