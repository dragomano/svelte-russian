...
С рунами мы можем использовать `$effect.pre`, который работает так же, как `$effect`, но выполняется до обновлений DOM, запланированных после него (см. раздел [$effect.pre](/runes/effect/#effectpre), где приведён точный порядок выполнения). Если мы явно обращаемся к `messages` внутри тела эффекта, он будет выполняться при каждом изменении `messages`, но _не_ при изменении `theme`.
