...
- `lib` содержит код вашей библиотеки (утилиты и компоненты), который можно импортировать через псевдоним [`#lib`](https://svelte.dev/docs/kit/$lib) или упаковать для распространения с помощью [`svelte-package`](/kit/advanced/packaging).
  - директории с именем `server` на любом уровне вложенности помечают весь содержащийся в них код как [доступный только на сервере](/kit/advanced/server-only-modules). SvelteKit не позволит импортировать такой код в клиентский код.
