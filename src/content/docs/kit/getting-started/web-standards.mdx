...
SvelteKit использует [`fetch`](https://developer.mozilla.org/ru/docs/Web/API/fetch) для получения данных из сети. Данный метод доступен в [хуках](https://svelte.dev/docs/kit/hooks), [серверных роутах](https://svelte.dev/docs/kit/routing#server) и браузере.

:::note
Особая версия `fetch` доступна в [`load`-функциях](https://svelte.dev/docs/kit/load), [серверных хуках](https://svelte.dev/docs/kit/hooks#Server-hooks) и [API-роутах](https://svelte.dev/docs/kit/routing#server) для прямого вызова эндпойнтов при SSR без HTTP-запросов, с сохранением авторизации. (Для авторизованных запросов вне `load` нужно явно передавать заголовки `cookie`/`authorization`). Также позволяет делать относительные запросы, тогда как обычный `fetch` требует полный URL.
:::
...
- `event.url` в [хуках](https://svelte.dev/docs/kit/hooks) и [серверных маршрутах](https://svelte.dev/docs/kit/routing#server)
- [`page.url`](https://svelte.dev/docs/kit/$app-state) на [страницах](https://svelte.dev/docs/kit/routing#page)
