...
SvelteKit использует [`fetch`](https://developer.mozilla.org/ru/docs/Web/API/fetch) для получения данных из сети. Данный метод доступен в [хуках](/kit/advanced/hooks), [серверных роутах](/kit/core-concepts/routing#server) и браузере.

:::note
Особая версия `fetch` доступна в [`load`-функциях](/kit/core-concepts/load), [серверных хуках](/kit/advanced/hooks/#серверные-хуки) и [API-роутах](/kit/core-concepts/routing#server) для прямого вызова эндпойнтов при SSR без HTTP-запросов, с сохранением авторизации. (Для авторизованных запросов вне `load` нужно явно передавать заголовки `cookie`/`authorization`). Также позволяет делать относительные запросы, тогда как обычный `fetch` требует полный URL.
:::
...
- `event.url` в [хуках](/kit/advanced/hooks) и [серверных маршрутах](/kit/core-concepts/routing#server)
