...
Если во время загрузки, рендеринга или в эндпойнте возникает [неожиданная ошибка](/kit/advanced/errors/#неожиданные-ошибки), эта функция будет вызвана с параметрами `error`, `event`, кодом статуса `status` и сообщением `message`. Это позволяет делать две вещи:
