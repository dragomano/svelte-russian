...
:::note
При переходе со страницы A на страницу B SvelteKit сохраняет компоненты, общие для обеих страниц. Подробнее см. [здесь](/kit/core-concepts/state-management/#состояние-компонентов-и-страниц-сохраняется).
:::
...
`+error.svelte` _не_ используется при ошибках в:
  - [`handle`](/kit/advanced/hooks/#handle)
  - обработчиках запросов [+server.js](#server)
:::

Подробнее об обработке ошибок можно прочитать [здесь](/kit/advanced/errors).
...
:::note
Файлы `+layout` не влияют на `+server.js`. Если вам нужно выполнить какую-то логику перед каждым запросом, добавьте её в хук [`handle`](/kit/advanced/hooks/#handle).
:::
...
- [Документация: Расширенная маршрутизация](/kit/advanced/advanced-routing)
