...
Подробное описание API доступно в разделе [`load`](https://svelte.dev/docs/kit/load).
...
- полное описание API доступно в разделе [`load`](https://svelte.dev/docs/kit/load)
...
Аналогично тому, как `+page.svelte` получает данные из `+page.js`, компонент `+layout.svelte` может получать данные из функции [`load`](https://svelte.dev/docs/kit/load) в `+layout.js`.
...
:::note
Часто данные макета остаются неизменными при навигации между страницами. SvelteKit интеллектуально перезапускает функции [`load`](https://svelte.dev/docs/kit/load) только при необходимости.
:::
