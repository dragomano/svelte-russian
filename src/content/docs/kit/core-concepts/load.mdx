...
Функция `load` в файле `+page.js` выполняется как на сервере, так и в браузере (если не используется `export const ssr = false`, в этом случае она [выполняется только в браузере](/kit/core-concepts/page-options#ssr)). Если ваша функция `load` должна _всегда_ выполняться на сервере (например, потому что использует приватные переменные окружения или обращается к базе данных), её следует поместить в файл `+page.server.js`.
...
Поведение можно настроить через [параметры страницы](/kit/core-concepts/page-options). При отключении [SSR](/kit/core-concepts/page-options#ssr) универсальные `load` функции _всегда_ выполняются на клиенте.

Если маршрут содержит и универсальные, и серверные `load` функции, серверная выполняется первой.

Функция `load` вызывается во время выполнения, кроме случаев [пререндеринга](/kit/core-concepts/page-options#prerender) — тогда она вызывается при сборке.
...
:::note
В некоторых средах эти данные получают из заголовков запроса при SSR. Например, при использовании [adapter-node](/kit/build-and-deploy/adapter-node) может потребоваться дополнительная настройка для корректного определения URL.
:::
...
- При SSR ответы автоматически встраиваются в HTML через перехват методов `text`, `json` и `arrayBuffer` объекта `Response`. Заголовки не сериализуются, если явно не указано в [`filterSerializedResponseHeaders`](/kit/advanced/hooks/#handle).
...
Другие куки не будут передаваться при `credentials: 'include'`, потому что SvelteKit не знает, к какому домену относится каждый куки (браузер не передаёт эту информацию), поэтому их передача небезопасна. Используйте [хук handleFetch](/kit/advanced/hooks/#handlefetch) для обхода этого ограничения.
...
Если во время выполнения `load` произойдёт ошибка, будет отображён ближайший [`+error.svelte`](/kit/core-concepts/routing#error). Для [_ожидаемых_](/kit/advanced/errors/#ожидаемые-ошибки) ошибок используйте хелпер `error` из `@sveltejs/kit`, чтобы указать HTTP-статус и опциональное сообщение:
...
При возникновении [_неожиданной_](/kit/advanced/errors/#неожиданные-ошибки) ошибки SvelteKit вызовет [`handleError`](/kit/advanced/hooks/#handleerror) и обработает её как 500 Internal Error.
...
- Взаимодействия с [`<form>`](/kit/core-concepts/form-actions/#get-против-post)
...
- Используйте [хуки](/kit/advanced/hooks) для защиты маршрутов до выполнения любых функций `load`
...
Теперь вы можете вызывать `requireLogin` в любой функции `load` (или, например, в [действиях формы](/kit/core-concepts/form-actions)), чтобы гарантировать, что пользователь авторизован:
