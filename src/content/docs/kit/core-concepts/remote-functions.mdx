...
Как аргументы, так и возвращаемые значения сериализуются с помощью [devalue](https://github.com/sveltejs/devalue), который поддерживает типы вроде `Date` и `Map` (а также пользовательские типы, определённые в вашем хуке [transport](https://svelte.dev/docs/kit/hooks#Universal-hooks-transport)) в дополнение к JSON.
