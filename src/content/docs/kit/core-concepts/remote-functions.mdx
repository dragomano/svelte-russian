...
:::note
Если `checkbox` не отмечен, его значение не включается в объект [`FormData`](https://developer.mozilla.org/en-US/docs/Web/API/FormData), из которого SvelteKit формирует данные. Поэтому в нашей схеме нужно задать для этого поля значение `false` по умолчанию, а не просто сделать его необязательным. В Valibot это означает использование `v.optional(v.boolean(), false)` вместо `v.boolean()`, а в Zod — `z.boolean().default(false)` вместо `z.boolean()`.
:::
