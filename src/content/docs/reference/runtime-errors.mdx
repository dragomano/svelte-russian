...
Ключевое выражение в блоке each с привязкой по ключам должно возвращать одно и то же значение при многократном вызове для одного и того же элемента. Использование выражений вида `[item.a, item.b]` создаёт новый массив при каждом вызове, а новый массив никогда не будет равен сам себе. Вместо этого используйте примитивное значение или создайте стабильный ключ, например: `item.a + '-' + item.b`.
...
Эффекты могут быть созданы только во время выполнения родительского эффекта. Это означает, что их нельзя создать, например, внутри обработчика событий или после выражения `await` (если только `await` не находится непосредственно внутри тега `<script>` компонента, а не внутри асинхронной функции).

В очень редких случаях уместно использовать [`$effect.root`](/runes/effect/#effectroot), чтобы иметь возможность создавать эффекты за пределами обычного жизненного цикла компонента.
...
<script>
  ...
  if (BROWSER) {
    // плохо! интерфейс останется полностью неинтерактивным, пока эта асинхронная операция не завершится
...
На некоторых платформах для включения этого API требуется установить специальные флаги конфигурации. Обратитесь к документации вашей платформы.
...
Вы (или используемый вами фреймворк) вызвали [`render(...)`](https://svelte.dev/docs/svelte/svelte-server#render) с компонентом, содержащим выражение `await`. Либо выполните `await` для результата `render`, либо оберните `await` (или компонент, содержащий его) в [`<svelte:boundary>`](/special-elements/svelte-boundary) с фрагментом `pending`.
...
Значение, переданное в проп `this` компонента `<svelte:element>`, должно быть допустимым именем HTML-элемента, SVG-элемента, MathML-элемента или именем пользовательского элемента. Было передано значение, содержащее недопустимые символы (например, пробелы или специальные символы), что может представлять угрозу безопасности. Убедитесь, что передаются только корректные имена тегов.
...
Эта ошибка возникает, когда `hydratable` используется несколько раз с одним и тем же ключом. Чтобы избежать её, вы можете:
...
<script>
  ...
  // какой из них должен «победить» и попасть в сериализованный ответ?
...
Некоторые функции, такие как `hydratable`, нельзя вызывать вне вызова `render(...)`, например на верхнем уровне модуля.
...
Утилита [`createContext()`](https://svelte.dev/docs/svelte/svelte#createContext) возвращает набор из трёх функций `[get, set, has]`. `get` вызовет ошибку, если `set` не использовалась для установки контекста в текущем компоненте или в одном из его предков.
