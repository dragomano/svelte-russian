...
Утилита [`createContext()`](https://svelte.dev/docs/svelte/svelte#createContext) возвращает пару функций `[get, set]`. `get` вызовет ошибку, если `set` не был использован для установки контекста в родительском компоненте.
