...
Обеспечьте, чтобы `onmouseover` и `onmouseout` сопровождались `onfocus` (или `onfocusin`) и `onblur` (или `onfocusout`) соответственно. Это гарантирует, что функциональность, активируемая этими событиями мыши, будет доступна и для пользователей клавиатуры.
