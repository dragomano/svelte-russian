...
Значение `$derived`, созданное внутри эффекта, перестанет обновляться, когда эффект будет уничтожен. Вам следует создать `$derived` вне эффекта или внутри `$effect.root`.
...
`unmount` был вызван с прокси состояния:
...
Избегайте использования `$state` здесь. Если `component` по какой-то причине должен быть реактивным, используйте вместо этого `$state.raw`.
...
Когда происходит ошибка при рендеринге содержимого [`<svelte:boundary>`](/special-elements/svelte-boundary), вызывается обработчик `onerror` с ошибкой и функцией `reset`, которая пытается повторно отобразить содержимое.

Эта функция `reset` должна вызываться только один раз. После этого она не имеет эффекта — в случае, когда ссылка на `reset` сохраняется вне границы, нажатие на кнопку, пока `<Contents />` отображается, _не_ приведёт к повторному рендерингу содержимого.
